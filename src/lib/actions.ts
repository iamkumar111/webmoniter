...
export async function createUser(data: z.infer<typeof UserSchema> & { password?: string; organizationId?: string; userGroupId?: string }) {
  ...
  const passwordToHash = data.password || "WebMoniter123!";
...
export async function testSlackWebhook(webhookUrl: string) {
  try {
    const response = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({
        text: "🧪 *WebMoniter Test Message*\n\nYour Slack integration is working correctly!",
        blocks: [
          {
            type: "section",
            text: {
              type: "mrkdwn",
              text: "🧪 *WebMoniter Test Message*\n\nYour Slack integration is working correctly! You will receive alerts here when your monitors go down or recover."
            }
          }
        ]
      })
...
export async function testDiscordWebhook(webhookUrl: string) {
  try {
    const response = await fetch(webhookUrl, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        content: "🧪 **WebMoniter Test Message**",
        embeds: [{
          ...
          footer: {
            text: "WebMoniter Uptime Monitoring"
          },
